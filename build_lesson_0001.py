...
下圖把每條線的「街」畫成灰色帶 —— 一眼就看出它們寬度差很多：
""")

code(r"""plt.scatter(X[:, 0], X[:, 1], c=y, cmap="coolwarm", s=60, edgecolors="k")

for m, b, half_width in [(1, 0.65, 0.33), (0.5, 1.6, 0.55), (-0.2, 2.9, 0.20)]:
    yfit = m * xfit + b
    plt.plot(xfit, yfit, "-k")
    plt.fill_between(xfit, yfit - half_width, yfit + half_width,
                     edgecolor="none", color="#888", alpha=0.35)

plt.xlim(-1, 3.5)
plt.title("Each line is really a 'street' — widths differ a lot")
plt.xlabel("x1"); plt.ylabel("x2")
plt.show()
""")
...
這個地基才真的是你的。下一格填好兩個函數，再下一格會**自動對答案**。
""")
...
print("\n整體:", "🎉 通過！你已經把 SVM 的幾何寫成程式了。" if (ok1 and ok2)
